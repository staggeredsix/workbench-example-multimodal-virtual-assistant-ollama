...
from chatui.utils import compile, database, logger, nim
...
# Helper class for Ollama integration
...
class OllamaChatModel(BaseChatModel):
    ...
    def list_models(self):
        ...
        url = f"{server_url}:{self.ollama_port}/api/tags"
    ...
    def pull_model(self, model_name):
        ...
        url = f"{server_url}:{self.ollama_port}/api/pull"
        ...
        try:
            # Use streaming for the pull request (standard for model pulls)
            ...
            for line in response.iter_lines():
                if line:
                    try:
                        ...
                        print(f"Pull progress: {progress}")
                    except json.JSONDecodeError:
                        print(f"Non-JSON progress: {line.decode('utf-8')}")
...
def build_page(client: chat_client.ChatClient) -> gr.Blocks:
    ...
    with gr.Blocks(title=TITLE, theme=kui_theme, css=kui_styles + _LOCAL_CSS) as page:
        ...
        ollama_router_model_state = gr.State("")
        ollama_retrieval_model_state = gr.State("")
        ollama_generator_model_state = gr.State("")
        ollama_hallucination_model_state = gr.State("")
        ollama_answer_model_state = gr.State("")
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column(scale=10, min_width=350) as settings_column:
                ...
                with gr.Tabs(selected=0) as settings_tabs:
                    ...
                    with gr.TabItem("Ollama", id=3, interactive=False, visible=False) as ollama_settings:
                        ...
                        with gr.Row():
                            ollama_model = gr.Textbox(
                                ...
                                placeholder="llama3",
                                value="llama3",
                                info="Default model name to use for all components"
                            )
                        ...
                        # Add component-specific model settings
                        gr.Markdown("### Component-Specific Models (Optional)")
                        gr.Markdown("You can specify different models for each component. If not specified, the default model will be used.")
                        
                        with gr.Row():
                            ollama_router_model = gr.Textbox(
                                label="Router Model",
                                placeholder="llama3",
                                value="",
                                info="Model to use for routing questions"
                            )
                            ollama_retrieval_model = gr.Textbox(
                                label="Retrieval Grader Model",
                                placeholder="llama3",
                                value="",
                                info="Model to use for grading document relevance"
                            )
                        
                        with gr.Row():
                            ollama_generator_model = gr.Textbox(
                                label="Generator Model",
                                placeholder="llama3",
                                value="",
                                info="Model to use for generating answers"
                            )
                            ollama_hallucination_model = gr.Textbox(
                                label="Hallucination Grader Model",
                                placeholder="llama3",
                                value="",
                                info="Model to use for checking hallucinations"
                            )
                        
                        with gr.Row():
                            ollama_answer_model = gr.Textbox(
                                label="Answer Grader Model",
                                placeholder="llama3",
                                value="",
                                info="Model to use for grading answer quality"
                            )
                    ...
                    with gr.TabItem("Monitor", id=4, interactive=False, visible=False) as console_settings:
                        gr.Markdown("")
                    ...
                    # Fourth tab item is for collapsing the entire settings pane for readability. 
                    with gr.TabItem("Hide All Settings", id=5, interactive=True, visible=True) as hide_all_settings:
                        gr.Markdown("")
        ...
        pdf_docs_upload.upload(_upload_pdf_documents, [pdf_docs_upload], [url_docs_clear, file_upload_clear, url_videos_clear, agentic_flow])
        ...
        def _update_ollama_state(use_ollama_val, server, port, model, 
                         router_model, retrieval_model, generator_model, 
                         hallucination_model, answer_model):
            return (use_ollama_val, server, port, model, 
                    router_model, retrieval_model, generator_model, 
                    hallucination_model, answer_model)
        
        # Connect Ollama UI components to functions
        use_ollama.change(_update_ollama_state, 
                 [use_ollama, ollama_server, ollama_port, ollama_model,
                  ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                  ollama_hallucination_model, ollama_answer_model], 
                 [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
                  ollama_router_model_state, ollama_retrieval_model_state, ollama_generator_model_state,
                  ollama_hallucination_model_state, ollama_answer_model_state])

        ollama_server.change(_update_ollama_state, 
                            [use_ollama, ollama_server, ollama_port, ollama_model,
                            ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                            ollama_hallucination_model, ollama_answer_model], 
                            [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        ollama_port.change(_update_ollama_state, 
                        [use_ollama, ollama_server, ollama_port, ollama_model,
                        ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                        ollama_hallucination_model, ollama_answer_model], 
                        [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        ollama_model.change(_update_ollama_state, 
                        [use_ollama, ollama_server, ollama_port, ollama_model,
                            ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                            ollama_hallucination_model, ollama_answer_model], 
                        [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        # Add event handlers for component-specific model fields
        ollama_router_model.change(_update_ollama_state, 
                                [use_ollama, ollama_server, ollama_port, ollama_model,
                                ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                                ollama_hallucination_model, ollama_answer_model], 
                                [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        ollama_retrieval_model.change(_update_ollama_state, 
                                    [use_ollama, ollama_server, ollama_port, ollama_model,
                                    ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                                    ollama_hallucination_model, ollama_answer_model], 
                                    [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        ollama_generator_model.change(_update_ollama_state, 
                                    [use_ollama, ollama_server, ollama_port, ollama_model,
                                    ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                                    ollama_hallucination_model, ollama_answer_model], 
                                    [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        ollama_hallucination_model.change(_update_ollama_state, 
                                        [use_ollama, ollama_server, ollama_port, ollama_model,
                                        ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                                        ollama_hallucination_model, ollama_answer_model], 
                                        [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        ollama_answer_model.change(_update_ollama_state, 
                                [use_ollama, ollama_server, ollama_port, ollama_model,
                                ollama_router_model, ollama_retrieval_model, ollama_generator_model,
                                ollama_hallucination_model, ollama_answer_model], 
                                [use_ollama_state, ollama_server_state, ollama_port_state, ollama_model_state,
        ...
        def _stream_predict(
            ...
            ollama_retrieval_model: str,
            ollama_generator_model: str,
            ollama_hallucination_model: str,
            ...
            inputs = {"question": question, 
                    ...
                    "ollama_retrieval_model": ollama_retrieval_model,
                    "ollama_generator_model": ollama_generator_model,
                    "ollama_hallucination_model": ollama_hallucination_model,
                    "ollama_answer_model": ollama_answer_model,
        ...
        sample_query_1.click(
            _my_build_stream, [sample_query_1, 
                            ...
                            ollama_port_state,
                            ollama_model_state,
                            ollama_router_model_state,
                            ollama_retrieval_model_state,
                            ollama_generator_model_state,
                            ollama_hallucination_model_state,
                            ollama_answer_model_state,
